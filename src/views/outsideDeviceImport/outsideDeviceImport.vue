  ...
  export default {
    ...
    data() {
      return {
        ...
        agentUsers: [],
        message:
          '请导入含外部编号、私钥的txt文档（注意：TXT格式，按【外部编号，私钥】格式），目前可导入金吉外部设备编号',
        colums: [
    ...
    methods: {
      ...
      async handleDownload() {
        ...
        let isAllReflect = true
        this.selectDevicesObjList.forEach((v, i) => {
          if (v.status !== 3) {
            isAllReflect = false
          }
        })
        if (!isAllReflect) {
          this.$message.error('只有映射成功才能下载！')
          return
        }
        const param = {
