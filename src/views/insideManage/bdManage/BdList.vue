<template>
  <div class="bd-container bd-manage">
...
export default {
  ...
  methods: {
    ...
    handleEdit(type) {
      ...
      if (type === 'add') { // 新增代理商
        // this.$route.
        this.$router.push({ name: 'addBD', query: { plan: 'private' }})
      } else {
        if (!checkIsNull()) {
          return false
        }
        if (type === 'edit') { // 编辑代理商
          this.$router.push({ name: 'editBD', query: { id: this.checkedRow.adUserId, phone: this.checkedRow.phone }})
        }
      }
    },
    handleDelete(row) {
